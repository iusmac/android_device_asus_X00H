 ...
 */
#define LOG_TAG "android.hardware.biometrics.fingerprint@2.1-service.X00H"
#define LOG_VERBOSE "android.hardware.biometrics.fingerprint@2.1-service.X00H"
...
fingerprint_device_t* getDeviceForVendor(const char *module_id)
{
    ...
    if (hw_mdl == nullptr) {
        ALOGE("No valid fingerprint module: module id %s", module_id);
        return nullptr;
    }
    ...
    if (module->common.methods->open == nullptr) {
        ALOGE("No valid open method: module id %s", module_id);
        return nullptr;
    }
    ...
    if (0 != (err = module->common.methods->open(hw_mdl, nullptr, &device))) {
        ALOGE("Can't open fingerprint methods, module id %s, error: %d", module_id, err);
        return nullptr;
    }
    ...
    ALOGI("Loaded fingerprint module: module id %s", module_id);
    return fp_device;
}
...
fingerprint_device_t* getFingerprintDevice()
{
    ...
    // cdfinger HAL
    fp_device = getDeviceForVendor("cdfinger.fingerprint");
    if (fp_device == nullptr) {
        ALOGE("Failed to load cdfinger fingerprint module");
    } else {
        setFpVendorProp("cdfinger");
        return fp_device;
    }

    // goodix HAL
    fp_device = getDeviceForVendor(FINGERPRINT_HARDWARE_MODULE_ID);
    if (fp_device == nullptr) {
